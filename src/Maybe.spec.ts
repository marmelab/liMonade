    ...
    describe('Nothing', () => {
        ...
        describe('Traversable Law', () => {
            it('Identity', async () => {
                await expect(
                    Nothing.of('value')
                        .map(Identity.of)
                        .sequence(Identity.of),
                ).toEqual(Identity.of(Nothing.of('value')));
            });

            it('Composition', async () => {
                const Compose = createCompose(Right, Nothing);
                expect(
                    Identity.of(Right.of(Nothing.of(true)))
                        .map(v => new Compose(v))
                        .sequence(Compose.of),
                ).toEqual(
                    new Compose(Identity.of(Right.of(Nothing.of(true)))
                        .sequence(Right.of)
                        .map(v => v.sequence(Nothing.of)) as Right<Nothing>),
                );
            });
            ...
            it('Naturality', () => {
                ...
                const a = Nothing.of(Just.of('value')).sequence(Just.of);
                expect(maybeToEither(a)).toEqual(
                    Nothing.of(Just.of('value'))
                        .map(maybeToEither)
                        .sequence(Right.of),
                );
            });
